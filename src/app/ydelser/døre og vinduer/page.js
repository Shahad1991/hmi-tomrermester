...
import { Hammer, Wrench, Drill, Ruler } from 'lucide-react';


export default async function DørVinduerPage() {
  try {
    ...
    const dørVinduerItems = allItems.filter(item => 
      item.categories && item.categories.some(category => 
        category.slug === 'dør-og-vinduer' || category.slug === 'dør og vinduer'
      )
    );
    ...
    return (
      <YdelseLayout
        heroImage="/images/services/dør-vinduer.jpeg"
        heroTitle="Døre & Vinduer"
